    ...
    async fn wait_until_flow_suspends(
        ...
        loop {
            ...
            if sqlx::query_scalar!(
                "SELECT suspend > 0 AS \"r!\" FROM v2_queue WHERE id = $1",
                flow
            )
            .fetch_one(db)
            .await
...
async fn completed_job(uuid: Uuid, db: &Pool<Postgres>) -> CompletedJob {
    sqlx::query_as::<_, CompletedJob>(
        "SELECT *, result->'wm_labels' as labels FROM v2_completed_job  WHERE id = $1",
    )
    .bind(uuid)
    .fetch_one(db)
    .await
...
async fn test_script_schedule_handlers(db: Pool<Postgres>) {
    ...
    in_test_worker(
        ...
        async move {
            ...
            let completed_job =
                sqlx::query!("SELECT script_path FROM v2_completed_job  WHERE id = $1", uuid)
                    .fetch_one(&db2)
                    .await
                    .unwrap();
