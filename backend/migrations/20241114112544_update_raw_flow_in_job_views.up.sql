...
SELECT
    ...
    queue.flow_status,
    case when queue.job_kind = 'flow' then (SELECT value FROM flow WHERE flow.path = queue.script_path AND flow.workspace_id = queue.workspace_id)
    else coalesce(queue.raw_flow, job.raw_flow) end as raw_flow,
    queue.is_flow_step,
    queue.language,
    queue.suspend,
    queue.suspend_until,
...
SELECT
    ...
    case when completed_job.job_kind = 'flow' then (SELECT value FROM flow WHERE flow.path = completed_job.script_path AND flow.workspace_id = completed_job.workspace_id)
    else coalesce(completed_job.raw_flow, job.raw_flow) end as raw_flow,
