...
async fn execute_component(
    ...
    let (policy, policy_triggerables) = match &mut payload {
        // 1. "preview" mode.
        ExecuteApp {
            ...
            &PolicyTriggerableInputs {
                static_inputs: std::mem::take(static_fields),
                one_of_inputs: std::mem::take(one_of_inputs),
                allow_user_resources: std::mem::take(allow_user_resources),
            },
        ),
        // 2. "run" mode.
        _ => {
            ...
            let policy = if let Some(id) = payload.version {
                let policy_fut = policy_fut.map_ok(Arc::new);
                arc_policy = cache::cached!(1000, i64 => Arc<Policy>, &id, policy_fut).await?;
                &*arc_policy
            } else {
            ...
            // - flow: `flow/<runnable_path>`
            // - script: `script/<runnable_path>`
            // - inline script: `rawscript/<digest(raw_code)>`
            let path = match &payload {
                // flow or script: just use the `runnable_path`.
                ...
                ExecuteApp { raw_code: Some(_), id: Some(id), .. } => {
                    ...
                    .fetch_one(&db)
                    .map_err(Into::<Error>::into);
                    &cache::cached!(10000, i64 => String, id, fut).await?
                }
                _ => unreachable!(),
            };
